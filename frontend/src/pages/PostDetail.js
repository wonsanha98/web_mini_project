...
export default function PostDetail() {
  ...
  if (error) return <p>{error}</p>;
  if (!post) return <p style={{ color: 'white' }}>⏳ 게시글을 불러오는 중...</p>;
  ...
  return (
    <div style={{
      ...
      <h2 style={{ color: 'skyblue' }}>{post.title}</h2>
      <p><strong>작성자:</strong> {post.author}</p>
      ...
      {post.user_id === currentUserId && (
        <div style={{ marginTop: '20px' }}>
          ...
          <button onClick={handleDelete} style={{ color: 'red' }}>🗑 삭제하기</button>
        </div>
      )}
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {comments.map((c, index) => (
          <li key={index} style={{ marginBottom: '15px' }}>
            <strong style={{ color: 'skyblue' }}>{c.author}</strong>: {editingCommentId === c.id ? (
              <>
                <textarea
                  value={editedContent}
                  onChange={(e) => setEditedContent(e.target.value)}
            ...
            {c.user_id && currentUserId && c.user_id === currentUserId && (
              <>
                <button
                  onClick={() => startEdit(c.id, c.content)}
                  style={{ marginLeft: '10px' }}> 수정</button>
                <button
                  onClick={() => handleCommentDelete(c.id)}
                  style={{ color: 'red', marginLeft: '10px' }}>🗑 삭제</button>
              </>
